...
class IngestionHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802 - required by BaseHTTPRequestHandler
        if self.path.startswith("/v1/records/") and self.path.endswith("/certify"):
            ...
            if not isinstance(actor, str) or not actor.strip() or policy_version not in {"st1-007-v1", "st1-023-historical-v1", "st1-026-source-attributed-v1"} or not persistence_enabled():
