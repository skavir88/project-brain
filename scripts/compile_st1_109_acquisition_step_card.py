#!/usr/bin/env python3
"""Compile an acquisition step card for the third write target.

This compiler is deterministic, local-only, and non-mutating. It reduces the
selected-class source-control verification surface plus existing execution
manifest data to only the minimal field-level payload and confirmations for
...
SOURCE_CONTROL_STEP_SCRIPT = ROOT / "scripts" / "compile_st1_108_source_control_verification_step_card.py"
...
def main() -> int:
    ...
    source_control_step = run_json(
        SOURCE_CONTROL_STEP_SCRIPT,
        [
    ...
    source_step = source_control_step["source_control_verification_step_card"]
    source_confirmations = source_step["required_confirmations"]
    source_ready = source_control_step["step_card_status"] == "READY_SOURCE_CONTROL_VERIFICATION_STEP_CARD"
    manifest_ready = manifest.get("compiler_result") == "READY_EXECUTION_MANIFEST"
    ready = source_ready and manifest_ready

    payload: dict[str, Any] = {}
    if manifest_ready:
        for row in manifest.get("execution_manifest", []):
            if row.get("target") == "ingestion.sdas_acquisition_events":
                payload = row.get("payload_shape", {})
                break

    output = {
        "schema_version": "st1-109-acquisition-step-card-v1",
        "candidate_class_id": source_control_step["candidate_class_id"],
        "project_scope": source_control_step["project_scope"],
        "step_card_status": "READY_ACQUISITION_STEP_CARD" if ready else "BLOCKED_ACQUISITION_STEP_CARD",
        "acquisition_step_card": {
            "write_target": "sdas_acquisition_events",
            "minimal_payload": payload if ready else {},
            "required_confirmations": {
                "source_control_verification_step_card_ready": source_ready,
