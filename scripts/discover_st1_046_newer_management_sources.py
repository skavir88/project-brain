#!/usr/bin/env python3
"""Find bounded metadata-only management-report candidates newer than 1402/06.
...
def newer(s:str)->bool:
 for y,m in DATE.findall(norm(s)):
  if (int(y),int(m))>(1402,6): return True
 return False

p=argparse.ArgumentParser();p.add_argument("--database",type=Path,default=runtime("pilot_metadata_index.sqlite"));p.add_argument("--output",type=Path,default=runtime("st1-046-newer-management-discovery.json"));p.add_argument("--limit",type=int,default=10);a=p.parse_args()
...
for parent,item in families.items():
 ...
 ranked.append({"alias":"st1-046-"+hashlib.sha256(parent.encode()).hexdigest()[:16],"relative_locator":parent,"score":score,"metadata_date_tokens":sorted(set(f"{y}/{int(m):02d}" for y,m in item["dates"])),"signals":{k:sorted(v) for k,v in item["signals"].items()},"file_count":len(files),"probeable_file_count":len(allowed),"extension_distribution":dict(sorted(Counter(x["extension"] for x in files).items())),"aggregate_size_bytes":size,"files":files})
ranked.sort(key=lambda x:(-x["score"],-x["probeable_file_count"],x["aggregate_size_bytes"],x["relative_locator"].casefold()))
out={"schema_version":"st1-046-newer-management-discovery-v1","generated_utc":datetime.now(UTC).isoformat(),"metadata_only":True,"threshold":"filename_or_directory_date_token > 1402/06","index_rows_queried":len(rows),"candidate_family_count":len(ranked),"candidates":ranked[:a.limit],"boundaries":{"content_opened":False,"new_smb_traversal":False,"external_model_use":False,"raw_output_outside_git":True}}
