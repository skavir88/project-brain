#!/usr/bin/env python3
"""Project only certified ST1-047 observations into Certified Knowledge."""
...
with psycopg.connect(host=os.environ['INGESTION_DB_HOST'],port=os.environ.get('INGESTION_DB_PORT','5432'),dbname=os.environ['INGESTION_DB_NAME'],user=os.environ['INGESTION_DB_USER'],password=os.environ['INGESTION_DB_PASSWORD']) as c:
 with c.cursor() as q:
  q.execute("""INSERT INTO ingestion.certified_knowledge_items (knowledge_id,source_fingerprint,source_record_id,certification_event_id,knowledge_text,provenance,certifying_actor,certification_timestamp,certification_policy_version,lifecycle_state) SELECT r.record_fingerprint,r.record_fingerprint,r.record_id,a.event_id,r.canonical_record->'payload'->>'statement',r.provenance,r.certification_actor,r.certification_timestamp,r.certification_policy_version,'certified' FROM ingestion.credibility_records r JOIN LATERAL (SELECT event_id FROM ingestion.certification_audit_events e WHERE e.record_fingerprint=r.record_fingerprint ORDER BY event_id DESC LIMIT 1) a ON true WHERE r.source_id='enterprise_ai_real_biweekly_management_observation' AND r.record_id LIKE 'st1-047-%' AND r.lifecycle_state='certified' ON CONFLICT (source_fingerprint) DO NOTHING RETURNING knowledge_id""")
  inserted=len(q.fetchall())
  q.execute("SELECT count(*) FROM ingestion.certified_knowledge_items WHERE source_record_id LIKE 'st1-047-%' AND certification_policy_version='st1-047-biweekly-management-report-v1'")
  total=q.fetchone()[0]
print(json.dumps({'inserted':inserted,'biweekly_management_knowledge_count':total}))'''
...
def main() -> int:
    ...
    if run.returncode:
        raise SystemExit(run.stderr.strip() or "projection failed")
