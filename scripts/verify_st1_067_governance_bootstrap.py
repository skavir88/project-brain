...
with psycopg.connect(host=os.environ['INGESTION_DB_HOST'], port=os.environ.get('INGESTION_DB_PORT', '5432'), dbname=os.environ['INGESTION_DB_NAME'], user=os.environ['INGESTION_DB_USER'], password=os.environ['INGESTION_DB_PASSWORD']) as conn:
  with conn.cursor() as cur:
    ...
    cur.execute("INSERT INTO ingestion.sdas_role_identity_verifications (actor_id,role_class,project_scope,evidence_reference,evidence_fingerprint,verification_method,evidence_quality,effective_from,event_hash) VALUES (%s,'CEO / Executive Governance Authority','synthetic_project','synthetic_test',%s,'synthetic','native',%s,%s),(%s,'synthetic project controls','synthetic_project','synthetic_test',%s,'synthetic','native',%s,%s)", (gov,h({'v':gov}),now,h({'e':gov}),pmo,h({'v':pmo}),now,h({'e':pmo})))
    ...
    cur.execute("INSERT INTO ingestion.sdas_source_control_verifications (source_id,accountable_actor_id,project_scope,document_data_class,business_time_rule,evidence_reference,evidence_fingerprint,evidence_quality,effective_from,event_hash) VALUES (%s,%s,'synthetic_project','synthetic_report','approved_report_header','synthetic_test',%s,'native',%s,%s)", (source,pmo,h({'v':source}),now,h({'e':source})))
